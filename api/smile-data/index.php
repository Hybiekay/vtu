<?php
    ...
    $oldbalance = (int) $userbalance;
    
    
    $deibt = $oldbalance - $amount;
    ...
    if($result['status']=="success"){
        ...
        $result = $controller->recordTransaction($userid,$servicename,$servicedesc,$amount,$userbalance,$body->ref,"0");
    ...
    elseif($result['status']=="processing"){
        if($refearedby <> ""){ $controller->creditReferalBonus($referal,$referalname,$refearedby,$servicename); }
        $controller->creditSmileBonus($userid,$amount,$servicename,$servicedesc);
        $result = $controller->recordTransaction($userid,$servicename,$servicedesc,$amount,$userbalance,$body->ref,"2");
        $response['status']="success";
        $response['network']="SMILE";
        $response['amount']=$result["amount"];
        $response['oldbal']=$result["oldbal"];
        ...
        $result = $controller->recordTransaction($userid,$servicename,$servicedesc,$amount,$userbalance,$body->ref,"1");
        echo json_encode($response);
        exit();
    }
