<?php
	...
	class AdminModel extends Model{
		...
		//Update Site Setting
		public function updateSiteSetting($sitename,$siteurl,$apidocumentation,$referalupgradebonus,$referalairtimebonus,$referaldatabonus,$referalwalletbonus,$referalcablebonus,$referalexambonus,$referalmeterbonus,$wallettowalletcharges,$agentupgrade,$vendorupgrade,$accountname,$accountno,$bankname,$electricity,$airtimemin,$airtimemax,$smilediscount,$kycoption,$kycbvncharges,$kycnincharges,$kycShouldEnable,$kycShouldVerify){
			$dbh=$this->connect();
			$sql="UPDATE sitesettings SET sitename=:sn,siteurl=:u,agentupgrade=:au,vendorupgrade=:vu,apidocumentation=:ad,referalupgradebonus=:rub,referalairtimebonus=:rab,referaldatabonus=:rdb,referalwalletbonus=:rwb,referalcablebonus=:rcb,referalexambonus=:reb,referalmeterbonus=:rmb,wallettowalletcharges=:wwc,accountname=:accna,accountno=:accno,bankname=:bnkna,electricitycharges=:electc,airtimemin=:amin,airtimemax=:amax,smilediscount=:smilediscount,kycOption=:kycO,kycBvnCharges=:kycB,kycNinCharges=:kycN,kycShouldEnable=:kycE,kycShouldVerify=:kycV WHERE sId=1";
			$query = $dbh->prepare($sql);
			...
			$query->bindParam(':smilediscount',$smilediscount,PDO::PARAM_STR);
			$query->bindParam(':kycO',$kycoption,PDO::PARAM_STR);
			$query->bindParam(':kycB',$kycbvncharges,PDO::PARAM_STR);
			$query->bindParam(':kycN',$kycnincharges,PDO::PARAM_STR);
			$query->bindParam(':kycE',$kycShouldEnable,PDO::PARAM_STR);
			$query->bindParam(':kycV',$kycShouldVerify,PDO::PARAM_STR);
			$query->execute();
			return 0;
		}
		...
		public function updateNetworkSetting($network,$general,$vtuStatus,$sharesellStatus,$airtimepin,$datapin,$sme,$sme2,$gifting,$corporate,$networkid,$vtuId,$sharesellId,$smeId,$smeId2,$giftingId,$corporateId,$momo,$manualorder,$coupon,$couponid){
			$dbh=$this->connect();
			$id = (float) $network;
			
			$sql="UPDATE networkid SET networkStatus=:g, vtuStatus=:vs, sharesellStatus=:sss, airtimepinStatus=:ap, datapinStatus=:dp, smeStatus=:s,sme2Status=:s2,giftingStatus=:gi, corporateStatus=:c, networkId=:nid, vtuId=:vtuid, sharesellId=:sharesellid,smeId=:smeid,smeId2=:smeid2,giftingId=:giftid,corporateId=:ccid,momoStatus=:momo,manualOrderStatus=:manualorder,couponStatus=:cp,couponid=:cpi WHERE nId = $id";
			$query = $dbh->prepare($sql); 
			...
			$query->bindParam(':s2',$sme2,PDO::PARAM_STR);
			...
			$query->bindParam(':smeid2',$smeId2,PDO::PARAM_STR);
			$query->bindParam(':ccid',$corporateId,PDO::PARAM_STR); 
			$query->bindParam(':giftid',$giftingId,PDO::PARAM_STR); 
			$query->bindParam(':momo',$momo,PDO::PARAM_STR); 
			$query->bindParam(':manualorder',$manualorder,PDO::PARAM_STR);
			$query->bindParam(':cp',$coupon,PDO::PARAM_STR);    
			$query->bindParam(':cpi',$couponid,PDO::PARAM_STR); 
			$query->execute();
