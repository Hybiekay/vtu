<?php
	...
	class SubscriberModel extends Model{
		...
		//----------------------------------------------------------------------------------------------------------------
		// GT BANK ACCOUNT MANAGEMENT
		//----------------------------------------------------------------------------------------------------------------
		
		//Generate GTBANK Account
		
		public function generateGtbankAccount($id){
			...
			if(empty($result->sGtBank)){
				...
				$monnifyStatus = $this->getConfigValue($d,"monifyStatus");
				$monnifyApi = $this->getConfigValue($d,"monifyApi");
				$monnifySecret = $this->getConfigValue($d,"monifySecrete");
				$monnifyContract = $this->getConfigValue($d,"monifyContract");
				

				//If Monnify Is Active, Create Virtual Account For User
				if($monnifyStatus == "On"){
					return $obj->createVirtualBankAccount4($id,$result->sFname,$result->sLname,$result->sPhone,$result->sEmail,$monnifyApi,$monnifySecret,$monnifyContract);
				}
